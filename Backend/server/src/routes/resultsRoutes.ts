...
router.get("/overview", authenticateJWT, async (req: Request, res: Response): Promise<void> => {
  ...
  if (!testId){res.status(400).json({ error: "Brak testId." }); return; }
  ...
  const tasks = await prisma.testTask.findMany({
    ...
    orderBy: { order: "asc" },
    select: { id: true, description: true, order: true, minPoints: true, maxPoints: true }
  });
  const k = tasks.length;
  if (k === 0) {
   res.json({
      header: { testId: test.id, testName: test.name, templateName: test.template.name, n: 0, totalMax: 0, pTest: 0 },
      students: [],
      items: [],
      summary: {}
    });
  ...
  if (n === 0) {
    res.json({
      header: { testId: test.id, testName: test.name, templateName: test.template.name, n: 0, totalMax, pTest: 0 },
      students: [],
      items: tasks.map(t => ({ ...t, p: 0, q: 1, f: 0, variance: 0, discrimination: 0, avgPoints: 0 })),
      summary: { mean: 0, median: 0, mode: null, min: 0, max: 0, range: 0, variance: 0, stdDev: 0, alpha: 0, stdError: 0 }
    }); return ;
  }
  ...
  const items = tasks.map(t => {
    ...
    const totalsCorrected = studentIds.map((sId, i) => totals[i] - (byStudent[sId][t.id] ?? 0));
  ...
  const alpha = (k > 1 && varTotal > 0)
    ? (k / (k - 1)) * (1 - (sumItemVar / varTotal))
    : 0;

  // łatwość testu = suma wszystkich punktów / (n * totalMax)
  const pTest = (n && totalMax) ? (totals.reduce((a, b) => a + b, 0) / (n * totalMax)) : 0;
